<%@ page language="java" contentType="text/html; charset=utf-8"
    pageEncoding="utf-8"%>
    
  <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.5.2/css/bootstrap.min.css"/>
 <c:set var="path" value="${pageContext.request.contextPath }" />

  <meta name="viewport" content="width=device-width, initial-scale=1"/>
  <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.5.2/css/bootstrap.min.css"/>
  <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js"></script>

  <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.16.0/umd/popper.min.js"></script>
  <script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.5.2/js/bootstrap.min.js"></script> 

<link rel="stylesheet"
   href="https://maxcdn.bootstrapcdn.com/bootstrap/4.5.2/css/bootstrap.min.css">


<!DOCTYPE html>
<html>


<section id="content" style="width: 100%;">

<div class="container mt-5">


  <!--Section: Content-->
  <div class="mb-5">

    <!-- Section heading -->
    <h3 class="text-center font-weight-bold mb-5">
Resume</h3>

  	<div class="row">

      <!--Grid column-->
      <div class="col-md-12">
        <!--News card-->
        <div class="card">
          <div class="card-body">
            <div class="row h-100 d-flex align-items-center">
              <div class="col-lg-6" >
              <div style="margin-left:200px;; font-size: 20px; ">
                <li class="mb-2">Name</li>
                <p  style="margin-top: -10px; font: bold;"> 인 하준 
                </p>
                   <li class="mb-2">Age</li>
                <p  style="margin-top: -10px; font: bold;"> 27
                </p>
                </div>
                <ul class="fa-ul mb-5 text-muted font-weight-light">
                
				<li class="mb-2"> <div class="mb-2" >각오</div></li>
                  <span class="fa-li"><i class="fas fa-check green-text"></i></span>도전정신이 투철함을 기반으로 끝임 없는 성장가능성을 가지고 있습니다, 주어진 일에 최선을 다하는 인재가 되겠습니다. 감사합니다
                  
                </ul>
                <div class="d-flex justify-content-between ml-3">
                 
                </div>
              </div>
              
              <div class="col-lg-6">

                <!--Carousel Wrapper-->
                 <img style="width: 100%;"
 						src="https://avatars.githubusercontent.com/u/71828303?s=400&u=91f2429020ee96bba401f5dde75eb6ebf32c8b65&v=4"
						class="img-fluid">
						
                    </div>
                

                  <!--Thumbnails-->
                  <a class="carousel-control-prev" href="#carousel-thumb" role="button" data-slide="prev">
                    <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                    <span class="sr-only">Previous</span>
                  </a>
                  <a class="carousel-control-next" href="#carousel-thumb" role="button" data-slide="next">
                    <span class="carousel-control-next-icon" aria-hidden="true"></span>
                    <span class="sr-only">Next</span>
                  </a>
                  <!--/.Thumbnails-->

                </div>
                <!--/.Carousel Wrapper-->
              </div>
            </div>
          </div>
        </div>
      </div>
      <!--Grid column-->

    </div>
  </div>
  <!--Section: Content-->


</div>








<div class="container py-5 my-5" style="text-align: center;width: 100%; justify-content: center;">
  <div class="p-md-3 mx-md-5 text-center text-lg-left">
    <h2 class="text-center font-weight-bold mb-4 pb-1">
Project / Resume</h2>
    <p class="text-center lead mb-5 pb-2 text-muted"></p>
    <div class="row">
      <div class="col-lg-4 mb-4">
        <div class="card">
          <div
            class="avatar w-100 white d-flex justify-content-center align-items-center"
          >
            <img
             src="http://jeonseongyeop.com/nixflos/img/directing/directing_05.jpg"
              
              class="img-fluid"
            />
          </div>
          <div class="card-body">
            <p class="font-weight-bold my-2">Semi project</p>
         
          </div>
          <div style="display: flex; justify-content: center;" >
          <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSnJ2SJB_P_H3nwllaHFbWvS_2EeOIxWWcLiw&usqp=CAU" usemap="#semi1" style="background-size: cover;width: 50Px; height: 50px; border-radius: 15px;border: solid;">
            <img src="https://play-lh.googleusercontent.com/nufRXPpDI9XP8mPdAvOoJULuBIH_OK4YbZZVu8i_-eDPulZpgb-Xp-EmI8Z53AlXHpqX" usemap="#semi2" style="background-size: cover;width: 50Px; height: 50px; border-radius: 15px;border: solid; margin-left: 5px;">
            <img src="https://img1.daumcdn.net/thumb/R800x0/?scode=mtistory2&fname=https%3A%2F%2Fblog.kakaocdn.net%2Fdn%2FQpdA1%2FbtqARXIcLjV%2FWfbkk3dh5nrc3aPd4yg3J1%2Fimg.gif" usemap="#semi3" style="background-size: cover;width: 50Px; height: 50px; border-radius: 15px;border: solid;margin-left: 5px;">
          </div>
           <map name="semi1">
        <area shape="rect" coords="00,00,150,300" href="http://rclass.iptime.org:9999/20PM_binbang/" alt="빈방"> 
       
    </map>
     <map name="semi2">
        <area shape="rect" coords="00,00,150,300" href="https://github.com/HajunIn/project/files/5897392/_.pdf" alt="포폴">
    </map>
     <map name="semi3">
        <area shape="rect" coords="00,00,150,300" href="https://github.com/kdyllll/binbang.git" alt="깃허브">
    </map>
          
        </div>
      </div>
      <div class="col-lg-4 mb-4">
        <div class="card">
          <div
            class="avatar w-100 white d-flex justify-content-center align-items-center"
          >
            <img
            	src="http://jeonseongyeop.com/nixflos/img/directing/directing_09.jpg"
              class="img-fluid"
            />
          </div>
          <div class="card-body">
            <p class="font-weight-bold my-2">Final project</p>
          
          </div>
           <div style="display: flex; justify-content: center;" ">
          <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS_3Cnoe0q-9r4Xsm62SwgmUAGbGKExzzaZUg&usqp=CAU" usemap="#final1" style="background-size: cover;width: 50Px; height: 50px; border-radius: 15px;border: solid;">
           <img src="https://play-lh.googleusercontent.com/nufRXPpDI9XP8mPdAvOoJULuBIH_OK4YbZZVu8i_-eDPulZpgb-Xp-EmI8Z53AlXHpqX" usemap="#final2" style="background-size: cover;width: 50Px; height: 50px; border-radius: 15px;border: solid; margin-left: 5px;">
            <img src="https://img1.daumcdn.net/thumb/R800x0/?scode=mtistory2&fname=https%3A%2F%2Fblog.kakaocdn.net%2Fdn%2FQpdA1%2FbtqARXIcLjV%2FWfbkk3dh5nrc3aPd4yg3J1%2Fimg.gif" usemap="#final3" style="background-size: cover;width: 50Px; height: 50px; border-radius: 15px;border: solid;margin-left: 5px;">
        </div>
         <map name="final1">
        <area shape="rect" coords="00,00,150,300" href="http://rclass.iptime.org:9999/20PM_JOBNOM_Final/" alt="잡파이브"> 
    </map>
     <map name="final2">
        <area shape="rect" coords="00,00,150,300" href="https://github.com/HajunIn/project/files/5897397/_.pdf" alt="포폴">
    </map>
     <map name="final3">
        <area shape="rect" coords="00,00,150,300" href="https://github.com/arangzzang/arang.final.github.io.git" alt="깃허브">
    </map>
      </div>
      </div>
      <div class="col-lg-4 mb-4">
      
      <!--   <div class="card">
          <div
            class="avatar w-100 white d-flex justify-content-center align-items-center"
          >
            <img
              src="http://jeonseongyeop.com/nixflos/img/directing/directing_02.jpg"
              class="img-fluid"
            />
          </div>
          <div class="card-body">
            <p class="font-weight-bold my-2">personal project</p>
        
          </div>
           <div style="display: flex; justify-content: center;" >
           
          <img usemap="#personal1" src="data:image/png;base64," style="background-size: cover;width: 50Px; height: 50px; border-radius: 15px;border: solid;">
           <img usemap="#personal2"  src="https://play-lh.googleusercontent.com/nufRXPpDI9XP8mPdAvOoJULuBIH_OK4YbZZVu8i_-eDPulZpgb-Xp-EmI8Z53AlXHpqX" style="background-size: cover;width: 50Px; height: 50px; border-radius: 15px;border: solid; margin-left: 5px;">
            <img usemap="#personal3" src="https://img1.daumcdn.net/thumb/R800x0/?scode=mtistory2&fname=https%3A%2F%2Fblog.kakaocdn.net%2Fdn%2FQpdA1%2FbtqARXIcLjV%2FWfbkk3dh5nrc3aPd4yg3J1%2Fimg.gif" style="background-size: cover;width: 50Px; height: 50px; border-radius: 15px;border: solid;margin-left: 5px;">
        </div>
        <map name="personal1">
        <area shape="rect" coords="00,00,150,300" href="http://13.209.129.60:8080/hajun/" alt="빈방"> 
    </map>
     <map name="personal2">
        <area shape="rect" coords="00,00,150,300" href="https://github.com/HajunIn/portfolio/files/5909266/_.docx" alt="포폴">
    </map>
     <map name="personal3">
        <area shape="rect" coords="00,00,150,300" href="https://github.com/HajunIn/portfolio.git" alt="깃허브">
    </map>
      </div> -->
      
  <!--     /////////////// -->
       <div class="card">
          <div
            class="avatar w-100 white d-flex justify-content-center align-items-center"
          >
            <img
              src="http://jeonseongyeop.com/nixflos/img/directing/directing_02.jpg"
              class="img-fluid"
            />
          </div>
          <div class="card-body">
            <p class="font-weight-bold my-2">지원 이력서</p>
        
          </div>
           <div style="display: flex; justify-content: center;" >
           
          <img usemap="#personal1" src="data:image/png;base64," style="background-size: cover;width: 50Px; height: 50px; border-radius: 15px;border: solid;">
           <img usemap="#personal2"  src="https://play-lh.googleusercontent.com/nufRXPpDI9XP8mPdAvOoJULuBIH_OK4YbZZVu8i_-eDPulZpgb-Xp-EmI8Z53AlXHpqX" style="background-size: cover;width: 50Px; height: 50px; border-radius: 15px;border: solid; margin-left: 5px;">
             <img usemap="#personal3" src="https://img1.daumcdn.net/thumb/R800x0/?scode=mtistory2&fname=https%3A%2F%2Fblog.kakaocdn.net%2Fdn%2FQpdA1%2FbtqARXIcLjV%2FWfbkk3dh5nrc3aPd4yg3J1%2Fimg.gif" style="background-size: cover;width: 50Px; height: 50px; border-radius: 15px;border: solid;margin-left: 5px;">
        </div>
        <map name="personal1">
        <area shape="rect" coords="00,00,150,300" href="http://13.209.129.60:8080/hajun/" alt="빈방"> 
    </map>
     <map name="personal2">
        <area shape="rect" coords="00,00,150,300" href="    https://github.com/HajunIn/project/files/5916761/_.docx" alt="포폴">
    </map>
    <map name="personal3">
        <area shape="rect" coords="00,00,150,300" href="    https://github.com/HajunIn/project/files/5916761/_.docx" alt="포폴">
    </map>
     
      </div>
      
      
      
      
      <map name="personal2">
    
        <area shape="rect" coords="00,00,150,300" href=" https://github.com/HajunIn/portfolio/files/5911593/_.pdf" alt="포폴">
    </map>
      
      
      
    </div>
  </div>
</div>
</section>
</html>

